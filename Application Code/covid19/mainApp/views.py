    ...
    if request.method == 'POST':
        ...
        if len(imgs) > 0:
            ...
            for x in imgs:
                ...
                if is_grey_scale("media/images/"+x):
                    out = test_pneumonia(imageFile, model1, model2)
                    res = out['Pneumonia']
                    typelis.append(res[0])
                    problis.append(res[1])
                else:
                    typelis.append('Not an X-ray')
                    problis.append('Not an X-ray')
            ...
            return render(request, 'mkit.html', {"show": 'Please Upload an image'})
    else:
...
def indiaAnalysis(request):
    plotsInd = []
    df_carona_in_india = pd.read_csv(
        "datasets/Covid_19_India/covid_19_india.csv")

    # Total cases of carona in India
    df_carona_in_india['Total Cases'] = df_carona_in_india['Cured'] + \
        df_carona_in_india['Deaths'] + df_carona_in_india['Confirmed']
    # Active cases of carona in India
    df_carona_in_india['Active Cases'] = df_carona_in_india['Total Cases'] - \
        df_carona_in_india['Cured'] - df_carona_in_india['Deaths']
    df_carona_in_india.head()
    # Till 10th April Active Cases in India
    df1 = df_carona_in_india[df_carona_in_india['Date'] == '10/04/20']
    fig = px.bar(df1, x='State/UnionTerritory', y='Active Cases',
                 color='Active Cases', barmode='group', height=600)
    ...
    df_carona_in_india['Date'] = pd.to_datetime(
        df_carona_in_india.Date, dayfirst=True)
    df_carona_in_india.head()
    # Daily Cases in India Datewise
    carona_data = df_carona_in_india.groupby(['Date'])['Total Cases'].sum(
    ).reset_index().sort_values('Total Cases', ascending=True)
    carona_data['Daily Cases'] = carona_data['Total Cases'].sub(
        carona_data['Total Cases'].shift())
    carona_data['Daily Cases'].iloc[0] = carona_data['Total Cases'].iloc[0]
    carona_data['Daily Cases'] = carona_data['Daily Cases'].astype(int)
    fig = px.bar(carona_data, y='Daily Cases', x='Date', hover_data=[
                 'Daily Cases'], color='Daily Cases', height=500)
    fig.update_layout(title='Daily Cases in India Datewise')
    plot_div = plot(fig, output_type='div', include_plotlyjs=False, show_link=False,
                    link_text="", image_width=500, config={"displaylogo": False})
    plotsInd.append(plot_div)

    carona_data['Corona Growth Rate'] = carona_data['Total Cases'].pct_change().mul(
        100).round(2)
    # Corona Growth Rate Comparison with Previous Day
    fig = px.bar(carona_data, y='Corona Growth Rate', x='Date', hover_data=[
                 'Corona Growth Rate', 'Total Cases'], height=500)
    fig.update_layout(
    ...
    # Moratality Rate
    carona_data = df_carona_in_india.groupby(['Date'])['Total Cases', 'Active Cases', 'Deaths'].sum(
    ).reset_index().sort_values('Date', ascending=False)
    carona_data['Mortality Rate'] = (
        (carona_data['Deaths']/carona_data['Total Cases'])*100)
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=carona_data['Date'], y=carona_data['Mortality Rate'], mode='lines+markers', name='Cases'))
    fig.update_layout(title_text='COVID-19 Mortality Rate in INDIA',
    ...
    carona_data = df_carona_in_india.groupby(['Date', 'State/UnionTerritory', 'Total Cases'])[
        'Cured', 'Deaths', 'Active Cases'].sum().reset_index().sort_values('Total Cases', ascending=False)
    fig = px.bar(carona_data, y='Total Cases', x='Date', hover_data=[
                 'State/UnionTerritory', 'Active Cases', 'Deaths', 'Cured'], color='Total Cases', barmode='group', height=700)
    ...
    carona_data = df_carona_in_india.groupby(['Date'])['Total Cases', 'Active Cases', 'Cured', 'Deaths'].sum(
    ...
    fig.update_layout(title_text='Curve Showing Different Cases from COVID-19 in India',
                      plot_bgcolor='rgb(225,230,255)')
    plot_div = plot(fig, output_type='div', include_plotlyjs=False, show_link=False,
                    link_text="", image_width=500, config={"displaylogo": False})
    plotsInd.append(plot_div)
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df['Dates'], y=df['ARIMA_Prediction'],
                             mode='lines+markers', name='Predicted Active Cases'))
    fig.update_layout(title_text='Curve Showing Predicted Active Cases from COVID-19 in India using ARIMA Model',
                      plot_bgcolor='rgb(225,230,255)')
